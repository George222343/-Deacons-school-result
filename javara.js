...
fetch("./total.json")
  ...
  .then((data) => {const student = data.find((item) => item.name.includes(passedValue))
    ...
    } else{nam2.innerText = student.alhan.toFixed(1)}
    if(student.taks === -1){
  ...
  copt.style = "display: block;";}
    ...
    if(student.className==="الانبا ميخائيل"||student.className==="الانبا بولا"||student.className==="المريمات 1"||student.className==="المريمات 2")
  ...
  kod.style = "display: block;";}
    ...
    const presenttt = Math.round((student.totalPresent/student.finalPresent)*100);
    if(presenttt < 50){nam9.style.color="red"
    fsf.style.color="red"
    }else{nam9.style.color="white"
    fsf.style.color="white"
  };
  const persa = Math.round((student.totalSubjects/student.finalSubjects)*100);
    if(student.totalSubjects=== -1){
        nam00.innerText="لا يوجد درجات للطالب";
    }else {  nam00.innerText = student.totalSubjects.toFixed(1)
    if(persa< 50){nam00.style.color="red"
    dss.style.color="red"}else{nam00.style.color="white"
    dss.style.color="white"}}
    nam01.innerText = student.Rehabilitation
    if(student.Rehabilitation==="غير مؤهلة"||student.Rehabilitation==="غير مؤهل"){moa.style.backgroundColor="red"} 
    else{moa.style.backgroundColor="darkgreen"}
